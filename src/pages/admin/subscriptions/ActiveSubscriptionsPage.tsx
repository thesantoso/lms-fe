...
import { mockActiveSubscriptions } from '@/data/subscriptions';
import type { ActiveSubscription } from '@/data/subscriptions';
...
const statusLabel = { active: 'Aktif', expiring: 'Segera Berakhir', expired: 'Berakhir' } as const;
const statusBadge = {
...
const ActiveSubscriptionsPage: React.FC = () => {
  const [subscriptions, setSubscriptions] = useState<ActiveSubscription[]>(mockActiveSubscriptions);
  ...
  const [modal, setModal] = useState<{ open: boolean; mode: ModalMode; subscription: ActiveSubscription | null }>({
    open: false,
    mode: 'create',
    subscription: null,
  });
  const [confirmDialog, setConfirmDialog] = useState<{ open: boolean; ids: string[]; schoolName?: string }>({
    open: false,
    ids: [],
  });
  ...
  const openCreate = () => setModal({ open: true, mode: 'create', subscription: null });
  const openEdit = (subscription: ActiveSubscription) =>
    setModal({ open: true, mode: 'edit', subscription });

  const handleSave = (data: Omit<ActiveSubscription, 'id'> & { id?: string }) => {
    setSaving(true);
    try {
      if (data.id) {
        setSubscriptions((prev) => prev.map((s) => (s.id === data.id ? { id: data.id, ...data } : s)));
      } else {
        const next: ActiveSubscription = { ...data, id: `sub-${Date.now()}` };
        setSubscriptions((prev) => [next, ...prev]);
      }
      setModal((m) => ({ ...m, open: false }));
      setNotice(data.id ? 'Langganan berhasil diperbarui!' : 'Langganan berhasil ditambahkan!');
    } finally {
      setSaving(false);
    }
  };

  const removeSubscriptions = async (ids: string[]) => {
    setLoading(true);
    ...
    try {
      ...
      setConfirmDialog({ open: false, ids: [], schoolName: undefined });
      setNotice(`${ids.length} langganan berhasil dihapus!`);
    } catch {
      setError('Gagal menghapus langganan. Coba lagi.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="flex items-center text-sm text-neutral-500">
          ...
          <CaretRight size={12} className="mx-2" />
          <span className="text-blue-600 font-medium">Langganan Aktif</span>
        </div>
      </div>
      ...
      <Card variant="elevated" className="border-none shadow-sm bg-white rounded-xl overflow-hidden">
        <CardBody className="p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <h2 className="text-xl font-bold text-neutral-900">Daftar Langganan</h2>
            <div className="flex items-center gap-3">
              <Button
                ...
                onClick={() =>
                  setConfirmDialog({ open: true, ids: Array.from(selectedIds), schoolName: undefined })
                }
              >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <div className="w-full sm:w-72">
              <Input
                placeholder="Cari sekolah atau paket..."
                value={searchTerm}
                onChange={(e) => {
            ...
            <table className="w-full min-w-[1000px]">
              <thead className="bg-neutral-100">
                <tr className="border-b border-neutral-200 text-left">
                  ...
                  <th className="px-4 py-3 text-sm font-bold text-neutral-800">Tanggal Mulai</th>
                  <th className="px-4 py-3 text-sm font-bold text-neutral-800">Tanggal Berakhir</th>
                  <th className="px-4 py-3 text-sm font-bold text-neutral-800">Sisa Masa Aktif</th>
              ...
              <tbody className="divide-y divide-neutral-100">
                ...
                ) : paginated.map((subscription, index) => {
                  const remaining = subscription.status === 'expired' ? null : daysLeft(subscription.endDate);
                  return (
                    <tr key={subscription.id} className="hover:bg-blue-50/30 transition-colors">
                      <td className="px-4 py-4 text-center">
                        <Checkbox
                          checked={selectedIds.has(subscription.id)}
                          onChange={(e) => toggleSelectOne(subscription.id, e.target.checked)}
                        />
                      </td>
                      <td className="px-4 py-4 text-sm text-neutral-600 font-medium">
                        {(currentPage - 1) * parseInt(entriesPerPage) + index + 1}
                      </td>
                      <td className="px-4 py-4 text-sm text-neutral-900 font-medium">{subscription.schoolName}</td>
                      <td className="px-4 py-4 text-sm text-neutral-600">{subscription.planName}</td>
                      <td className="px-4 py-4 text-sm text-neutral-600">{formatDate(subscription.startDate)}</td>
                      <td className="px-4 py-4 text-sm text-neutral-600">{formatDate(subscription.endDate)}</td>
                      <td className="px-4 py-4 text-sm text-neutral-600">
                        {subscription.status === 'expired' || (remaining !== null && remaining <= 0) ? (
                          <span className="text-neutral-400">Berakhir</span>
                        ) : (
                          <span className={remaining !== null && remaining <= 30 ? 'text-amber-700 font-medium' : 'text-neutral-600'}>
                            {remaining} hari
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${statusBadge[subscription.status]}`}
                        >
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              setConfirmDialog({ open: true, ids: [subscription.id], schoolName: subscription.schoolName })
                            }
                          >
                ...
                {!loading && filtered.length === 0 && (
                  <tr>
                    <td colSpan={9} className="px-4 py-12 text-center text-neutral-500">
                      {searchTerm
                        ? 'Tidak ada langganan yang cocok dengan pencarian'
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
            <p className="text-sm text-neutral-500">
              ...
              dari <span className="font-medium text-neutral-900">{filtered.length}</span> langganan
            </p>
      ...
      <Modal
        ...
        onClose={() => setConfirmDialog({ open: false, ids: [], schoolName: undefined })}
        size="sm"
      >
        <div className="flex flex-col items-center text-center py-4">
          ...
          <p className="text-neutral-500 text-sm mb-6">
            {confirmDialog.ids.length > 1
              ? `${confirmDialog.ids.length} langganan akan dihapus.`
              : confirmDialog.schoolName
                ? `Langganan "${confirmDialog.schoolName}" akan dihapus dan sekolah kehilangan akses paket.`
                : 'Langganan akan dihapus.'}
          </p>
          <div className="flex items-center gap-3 w-full">
            <Button
              variant="ghost"
              className="flex-1 bg-neutral-100 hover:bg-neutral-200 text-neutral-700 h-11 font-semibold rounded-lg"
              onClick={() => setConfirmDialog({ open: false, ids: [], schoolName: undefined })}
            >
              Batal
            </Button>
            <Button
              className="flex-1 bg-red-500 hover:bg-red-600 text-white h-11 font-semibold rounded-lg"
              onClick={() => removeSubscriptions(confirmDialog.ids)}
            >
              Hapus
            </Button>
          </div>
