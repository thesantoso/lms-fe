...
const EditClassModal: React.FC<EditClassModalProps> = ({ isOpen, onClose, onSave, classData }) => {
  ...
  useEffect(() => {
    if (isOpen && classData) {
      setFormData({
        name: classData.name || '',
        homeroomTeacherId: classData.homeroomTeacher || '', // Note: In real app this would be ID
  ...
  // Derived data
  const teachers = mockTeachers.map(t => ({ value: t.name, label: t.name })); // using name for now to match mock data
  ...
  return (
    ...
    >
      <div className="max-h-[80vh] overflow-y-auto pr-2">
        <p className="text-sm text-neutral-500 mb-6">Perbarui data kelas</p>
